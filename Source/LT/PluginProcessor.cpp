...
void JuceDemoPluginAudioProcessor::loadAudio(File fileToLoad) {
    
	DBG ("loop - loading audio");
	
	if ( ! fileToLoad.existsAsFile()) return; 
	
    AudioFormatManager* formatManager = new AudioFormatManager();
    
	//if (formatManager == 0) return;
	...
	//const ScopedLock myLock (getCurrentProcessor()->getCallbackLock());
    
	// set LENGTH
	int length = reader->lengthInSamples; 
	
	// resize if necessary 
	sampleBuffer = new AudioSampleBuffer(2, length); // resized dynamically when files are loaded ...
	sampleBuffer->readFromAudioReader(reader, 0, length, 0, true, true);
	
	reader->input->setPosition(0); // note ** these are /2 to get in the middle of the circular buffer (far from the beginning as  
    ...
    DBG (String("already have master"));
...
void JuceDemoPluginAudioProcessor::prepareToPlay (double sampleRate, int /*samplesPerBlock*/)
{
    ...
    File fileToLoad("/Users/aaronleese/Desktop/code/ENMFP/billie.raw");
    
    loadAudio(fileToLoad);
    
    
    jassert(sampleBuffer != 0);
        ...
        // Codegen(const float* pcm, uint numSamples, int start_offset, bool low_rank);
    Codegen * pCodegen = new Codegen(sampleBuffer->getSampleData(0,0), sampleBuffer->getNumSamples(), 0, false);
...
void JuceDemoPluginAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    myAudioPlayer->processBlock(buffer, midiMessages);
